...
const PageUsuario = () => {
  ...
  // Funcion que se ejecuta al apretar el boton guardar de editar usuario.
  const handleSaveUserEdited = async (data, legajo) => {
    try {
        //console.log("modificadoooooo",data);
        ...
        if (respUpdate) {
          Swal.fire({
            ...
            getUsuarios(); // Actualizo la tabla con el usuario recien editado.
        } else {
            
            Swal.fire(`Error: "Hubo un problema con la edicion."}`, "error");
        }
    } catch (error) {
  ...
  return (
    ...
    >
        {loading ? (
          ...
          <Typography>No hay datos disponibles para la ruta: /usuarios</Typography>
        )}
